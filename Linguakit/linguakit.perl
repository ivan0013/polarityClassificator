...
if ( !eval { require PerlIO::gzip; } ) {
    warn
"WARNING PerlIO::gzip module not installed, use 'cpan PerlIO::gzip' to solve\n";
}
if ( !eval { require LWP::UserAgent; } ) {
    warn
"WARNING LWP::UserAgent module not installed, use 'cpan LWP::UserAgent' to solve\n";
}
if ( !eval { require HTTP::Request::Common; } ) {
    warn
"WARNING HTTP::Request::Common module not installed, use 'cpan HTTP::Request::Common' to solve\n";
}
exit 1 if !$deps;
...
my @common_args = (
    ...
    [
        ...
        metavar => "<input>",
        help =>
          "Path of the input(a plain text file or gz/zip) by default STDIN"
    ],
    [
        '-s',
        type => 'Bool',
        dest => 'string',
        help => "Changes input from a file path to a text string"
    ]
);
...
$dep->add_argument(
    ...
    type => 'Bool',
    dest => 'a',
    help => "Simple dependency analysis (by default syntactic output"
);
$dep->add_argument(
    '-fa',
    type => 'Bool',
    dest => 'fa',
    help => "Full dependency analysis"
);
$dep->add_argument(
    '-c',
    type => 'Bool',
    dest => 'c',
    help => "Tagged text with syntactic information (for correction rules)"
);
$dep->add_argument(
    '-conll',
    type => 'Bool',
    dest => 'conll',
    help => "CoNLL output style"
);
...
$tagger->add_argument(
    ...
    type => 'Bool',
    dest => 'noner',
    help => "No NER or NEC is processed (by default PoS tagger output)"
);
$tagger->add_argument(
    '-ner',
    type => 'Bool',
    dest => 'ner',
    help =>
"PoS tagger with Named Entity Recognition - NER (only with 'tagger' module)"
);
$tagger->add_argument(
    '-nec',
    type => 'Bool',
    dest => 'nec',
    help =>
"PoS tagger with Named Entity Classification - NEC (only with 'tagger' module)"
);
...
$mwe->add_argument(
    ...
    type => 'Bool',
    dest => 'chi',
    help => "Chi-square co-occurrence measure (by default)"
);
$mwe->add_argument(
    '-log',
    type => 'Bool',
...
$mwe->add_argument(
    ...
    type => 'Bool',
    dest => 'scp',
    help => "Symmetrical conditional probability"
);
$mwe->add_argument(
    '-mi',
    type => 'Bool',
    dest => 'mi',
    help => "Mutual information "
);
$mwe->add_argument(
    '-cooc',
    type => 'Bool',
    dest => 'cooc',
    help => "Co-occurrence counting"
);
...
$tok->add_argument(
    ...
    type => 'Bool',
    dest => 'split',
    help => "Tokenization with splitting"
);
$tok->add_argument(
    '-sort',
    type => 'Bool',
    dest => 'sort',
    help => "Tokenization with tokens sorted by frequency"
);
...
my $kwic =
  $parser->add_parser( 'kwic', help => "Keyword in context (concordances)" );
...
my $link = $parser->add_parser( 'link',
    help => "Entity linking and semantic annotation" );
$link->add_args(@common_args);
$link->add_argument(
    '-json',
    type => 'Bool',
    dest => 'json',
    help => "Json output format of entity linking (by default)"
);
$link->add_argument(
    '-xml',
    type => 'Bool',
    dest => 'xml',
    help => "Xml output format of entity linking"
);
...
$sum->add_argument(
    ...
    dest => 'percentage',
    help =>
      "Percentage of the input text that will be summarized (by default 10%)"
);
...
$conj->add_argument(
    ...
    type => 'Bool',
    dest => 'pe',
    help => "The verb conjugator uses European Portuguese (by default)"
);
$conj->add_argument(
    '-pb',
    type => 'Bool',
    dest => 'pb',
    help => "The verb conjugator uses Brasilian Portuguese"
);
$conj->add_argument(
    '-pen',
    type => 'Bool',
    dest => 'pen',
    help =>
      "The verb conjugator uses European Portuguese before the spell agreement"
);
$conj->add_argument(
    '-pbn',
    type => 'Bool',
    dest => 'pbn',
    help =>
      "The verb conjugator uses Brasilian Portuguese before the spell agreement"
);

my $coref =
  $parser->add_parser( 'coref', help => "Named entity coreference solver" );
$coref->add_args(@common_args);
$coref->add_argument(
    '-crnec',
    type => 'Bool',
    dest => 'crnec',
    help => "NEC correction with NE Coreference Resolution"
);
...
#	}
    ...
    if ( $args->ner ) {    ##PoS tagging with ner
        do $NER;
        while ( my $line = <$input> ) {
            my $list = Tagger::tagger(
                Ner::ner(
                    Splitter::splitter(
                        Tokens::tokens( Sentences::sentences( [$line] ) )
                    )
                )
            );
            for my $result ( @{$list} ) {
...
#my $list = Keywords::keywords(Tagger::tagger(Ner::ner(Splitter::splitter(Tokens::tokens(Sentences::sentences([$line]))))));
    ...
    while ( my $line = <$input> ) {
        $ling =
          LanRecog::langrecog(
            Tokens::tokens( Sentences::sentences( [$line] ) ), \%Peso );
    }
    print "$ling\n";
